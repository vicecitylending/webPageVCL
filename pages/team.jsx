...
const MembersCarousel = dynamic(() => import("../components/carousel"), { ssr: false });
...
const getStaticProps = async ({ params, locale, previewData }) => {
  ...
  return {
    props: {
      seo: await safe(getPrismicData('seo', locale)),
      generalInformation: await safe(getPrismicData('general_information', locale)),
      menuContent: await safe(getPrismicData('menu', locale)),
      members: await safe(getPrismicCustomTypeData('member', locale)),
      footerContent: await safe(getPrismicData('footer', locale)),
      meetTeamContent: await safe(getPrismicData('meet_team', locale)),
      RealtorsResourceContent: await safe(getPrismicData('realtors_resource', locale)),
      PreApprovalProcessContent: await safe(getPrismicData('pre_approval_process', locale)),
      BuyingRefinancingProcessContent: await safe(getPrismicData('buying_refinancing_process', locale)),
      signUpContent: await safe(getPrismicData('sign_up', locale)),
      CirclesGridContent: await safe(getPrismicData('circle_grid_info', locale)),
      SimulatorContent: await safe(getPrismicData('simulator', locale)),
      locales: locales,
      locale: locale
    }
  }
